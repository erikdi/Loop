...
extension AbsorptionSpeed {
    ...
    var seconds : TimeInterval {
        ...
        switch(self) {
        case .ultraFast:
            return round(multiplier * defaultTimes.fast * 0.75)
        case .fast:
            return round(multiplier * defaultTimes.fast)
        case .normal:
            return round(multiplier * defaultTimes.medium)
        case .slow:
            return round(multiplier * defaultTimes.slow)
        }
    }
}
...
struct FoodPick : CustomStringConvertible {
    ...
    var carbs : Double {
        return round(item.carbPortion * ratio)
    }
    ...
    var description : String {
        ...
        return "\(displayPortion)g \(item.title) img \(String(describing: imageIdentifier)): \(displayCarbs)g KH (\(cr)%)"
    }
...
typealias FoodStats = [String:[String:Int]]
...
final class FoodManager {
    ...
    init() {
        ...
        if let pop = sections.firstIndex(of: popKey) {
...
extension UserDefaults {
    var foodStats : FoodStats {
        get {
            if let rawValue = dictionary(forKey: "com.loopkit.Loop.foodStats") {
        ...
        set {
            set(newValue, forKey: "com.loopkit.Loop.foodStats")
        }
    }
}
